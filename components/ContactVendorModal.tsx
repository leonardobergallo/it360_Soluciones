  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/contacto-vendedor', {
        ...
        body: JSON.stringify({
          ...formData,
          producto: productosInfo,
          precio: precioTotal,
          tipoConsulta: selectedProducts && selectedProducts.length > 1 ? 'Consulta de Múltiples Productos' : 'Consulta de Producto'
        }),
      });
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      ...
      <div className="bg-white rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="p-6">
          ...
          {selectedProducts && selectedProducts.length > 0 ? (
            <div className="bg-gradient-to-br from-cyan-50 to-blue-50 p-4 rounded-xl mb-6 border border-cyan-200">
              <h3 className="font-semibold text-gray-800 mb-3">Productos Seleccionados ({selectedProducts.length})</h3>
              <div className="space-y-2 mb-3">
                {selectedProducts.map((prod, index) => (
                  <div key={index} className="flex justify-between items-center p-2 bg-white rounded-lg">
                    ...
                    <span className="text-sm font-bold text-cyan-600">${prod.price.toLocaleString()}</span>
                  </div>
                ))}
              </div>
          ...
          <div className="mb-6">
            <button
              ...
              className="w-full bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white py-3 px-4 rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 flex items-center justify-center gap-3"
            >
